...
import { BarChart2, TrendingDown, Home, Users, ArrowRight, Check } from "lucide-react";
...
export function RunwayCommandCentrePreview() {
  ...
  return (
    <div
      ...
      data-testid="runway-command-centre"
    >
      {/* Title bar */}
      <div
      ...
      <div className="px-3 pb-2 flex items-baseline gap-1.5">
        ...
        <span className="text-[10px] ml-1" style={{ color: "hsl(38 72% 60%)" }}>
          — {s.sub}
        </span>
      </div>
      ...
      <div
        className="relative mx-3 mb-2.5 rounded-xl overflow-hidden"
        ...
        <svg viewBox="0 0 300 100" className="w-full" style={{ height: 100 }}>
          ...
          {[25, 50, 75].map((y) => (
            <line key={y} x1="0" y1={y} x2="300" y2={y}
              stroke="hsl(215 30% 16%)" strokeWidth="0.5" />
          ))}
          {/* Month markers */}
          {[0, 6, 12, 18].map((m) => (
            <text key={m} x={m === 18 ? 296 : m * 300 / 18} y="98"
              fontSize="5" fill="hsl(215 15% 30%)" textAnchor={m === 18 ? "end" : m === 0 ? "start" : "middle"}>
              {m}m
            </text>
          ))}
          {/* Area fill */}
          ...
          <text x="3" y="81" fontSize="5.5" fill="hsl(38 72% 55%)" opacity="0.6">
            Survival horizon
          </text>
        </svg>
        {/* Annotation chip */}
      ...
      {/* Sensitivity ranking */}
      <div
        className="mx-3 mb-2.5 rounded-xl p-3"
        style={{ background: "hsl(215 45% 11%)", border: "1px solid hsl(215 28% 16%)" }}
      >
        <p
          className="text-[8px] font-semibold uppercase tracking-widest mb-2.5"
      ...
      {/* Mobile simplified view — shown only on small screens as overlay */}
